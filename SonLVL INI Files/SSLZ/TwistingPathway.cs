...
namespace S3KObjectDefinitions.SSLZ
{
	class TwistingPathway : ObjectDefinition
	{
		...
		public override void Init(ObjectData data)
		{
			...
			int top = 0;
			int bottom = 0;
			for (int i = 0; i < Obj_Corkscrew_CosineTable.Length; i++)
			{
				top = Math.Min(Obj_Corkscrew_CosineTable[i], top);
				bottom = Math.Max(Obj_Corkscrew_CosineTable[i], bottom);
			}
			Point offset = new Point(left, top);
			BitmapBits bmp = new BitmapBits(right - left, bottom - top);
			for (int x = 0; x < 0x1A0; x++)
			{
				int y = Obj_Corkscrew_CosineTable[x] - top;
		...
		private int[] Obj_Corkscrew_CosineTable = {
